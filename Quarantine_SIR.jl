...
Pkg.add("MAT")
using MAT
vars = matread("/Users/rajd/Downloads/Hubei_Track.mat")
...
function loss_adjoint()
 ...
 loss = sum(abs2, log.(abs.(Infected[1, :])) .- log.(abs.(prediction[2, :]))) + sum(abs2, log.(abs.(Recovered[1, :]) .+ 1) .- log.(abs.(prediction[3, :] .+ 1)))
end
...
cb = function()
  ...
  scatter(Time', I, xaxis = "Time(Days)", yaxis = "Wuhan - Number", label = "Data: Infected", legend = :topleft, framestyle = :box, left_margin = 5mm)
  ...
  scatter!(Time', R, xaxis = "Time(Days)", yaxis = "Wuhan - Number", label = "Data: Recovered", legend = :topleft, framestyle = :box, left_margin = 5mm)
...
gif(anim,"Nature_Quarantine_NN_SIR_full.gif", fps=15)
...
Pkg.add("JLD")
using JLD
save("Mac_RD_Nature_QSIR.jld",  "Q_parameter", Q_parameter , "S_NN_all_loss", S_NN_all_loss, "I_NN_all_loss", I_NN_all_loss, "R_NN_all_loss", R_NN_all_loss, "T_NN_all_loss", T_NN_all_loss, "t", t, "Parameters", p3, "Loss", Loss, "P1", P1, "P2", P2)
...
scatter(Time', Infected[1, :], yaxis = "Wuhan: Number of cases", label = "Data: Infected", legend = :topleft, framestyle = :box, left_margin = 5mm, color = :red)
...
scatter!(Time', Recovered[1, :], yaxis = "Wuhan: Number of cases", label = "Data: Recovered", legend = :topleft, framestyle = :box, left_margin = 5mm, color = :blue)
...
savefig("RD_Nature_QSIR_10n.pdf")
...
savefig("RD_Nature_QSIR_11n.pdf")
...
savefig("RD_Nature_QSIR_12n.pdf")
